...
Public Class FrmEmailSignatures
    ...
    Public Sub New()
        ...
        _Filter = New EmailSignatureFilter(DgvData, PgFilter)
        _Filter.Filter()
    ...
    Private Sub BtnInclude_Click(sender As Object, e As EventArgs) Handles BtnInclude.Click
        Dim Form As New FrmEmailSignature(New EmailSignature, Me)
        Form.ShowDialog()
    End Sub
    Private Sub BtnEdit_Click(sender As Object, e As EventArgs) Handles BtnEdit.Click
        Dim EmailSignatureForm As FrmEmailSignature
        If DgvData.SelectedRows.Count = 1 Then
            Try
                Cursor = Cursors.WaitCursor
                _EmailSignature = New EmailSignature().Load(DgvData.SelectedRows(0).Cells("id").Value, True)
                EmailSignatureForm = New FrmEmailSignature(_EmailSignature, Me)
                EmailSignatureForm.ShowDialog()
            Catch ex As Exception
                CMessageBox.Show("ERRO ES007", "Ocorreu um erro ao carregar o registro.", CMessageBoxType.Error, CMessageBoxButtons.OK, ex)
            Finally
    ...
    Private Sub BtnDelete_Click(sender As Object, e As EventArgs) Handles BtnDelete.Click
        If DgvData.SelectedRows.Count = 1 Then
            Try
                Cursor = Cursors.WaitCursor
                _EmailSignature.Load(DgvData.SelectedRows(0).Cells("id").Value, False)
                If Not _EmailSignature.LockInfo.IsLocked Then
                    If CMessageBox.Show("O registro selecionado será excluído. Deseja continuar?", CMessageBoxType.Question, CMessageBoxButtons.YesNo) = DialogResult.Yes Then
                        ...
                        Catch ex As MySqlException
                            ...
                            Else
                                CMessageBox.Show("ERRO ES008", "Ocorreu um erro ao excluir o registro.", CMessageBoxType.Error, CMessageBoxButtons.OK, ex)
                            End If
                        End Try
                    End If
                Else
