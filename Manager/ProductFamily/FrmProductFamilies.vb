...
Public Class FrmProductFamilies
    ...
    Public Sub New()
        ...
        BtnInclude.Visible = Locator.GetInstance(Of Session).User.Privilege.ProductFamilyWrite
        BtnEdit.Visible = Locator.GetInstance(Of Session).User.Privilege.ProductFamilyWrite
        BtnDelete.Visible = Locator.GetInstance(Of Session).User.Privilege.ProductFamilyDelete
    ...
    Private Sub BtnInclude_Click(sender As Object, e As EventArgs) Handles BtnInclude.Click
        Dim Form As New FrmProductFamily(New ProductFamily, Me)
        Form.ShowDialog()
    End Sub
    Private Sub BtnEdit_Click(sender As Object, e As EventArgs) Handles BtnEdit.Click
        Dim ProductsFamilyForm As FrmProductFamily
        If DgvData.SelectedRows.Count = 1 Then
            Try
                Cursor = Cursors.WaitCursor
                _ProductsFamily = New ProductFamily().Load(DgvData.SelectedRows(0).Cells("id").Value, True)
                ProductsFamilyForm = New FrmProductFamily(_ProductsFamily, Me)
                ProductsFamilyForm.ShowDialog()
            Catch ex As Exception
                CMessageBox.Show("ERRO PF001", "Ocorreu um erro ao carregar o registro.", CMessageBoxType.Error, CMessageBoxButtons.OK, ex)
            Finally
    ...
    Private Sub BtnDelete_Click(sender As Object, e As EventArgs) Handles BtnDelete.Click
        If DgvData.SelectedRows.Count = 1 Then
            Try
                Cursor = Cursors.WaitCursor
                _ProductsFamily.Load(DgvData.SelectedRows(0).Cells("id").Value, False)
                If Not _ProductsFamily.LockInfo.IsLocked Then
                    If CMessageBox.Show("O registro selecionado será excluído. Deseja continuar?", CMessageBoxType.Question, CMessageBoxButtons.YesNo) = DialogResult.Yes Then
                        ...
                        Catch ex As MySqlException
                            ...
                            Else
                                CMessageBox.Show("ERRO PF002", "Ocorreu um erro ao excluir o registro.", CMessageBoxType.Error, CMessageBoxButtons.OK, ex)
                            End If
                        End Try
                    End If
                Else
