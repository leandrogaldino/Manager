...
Public Class FrmProductGroups
    Private _ProductsGroup As New ProductGroup
    ...
    Public Sub New()
        ...
        _Filter = New ProductGroupFilter(DgvData, PgFilter)
        _Filter.Filter()
        PgFilter.SelectedObject = _Filter
        BtnInclude.Visible = Locator.GetInstance(Of Session).User.Privileges.ProductGroupWrite
        BtnEdit.Visible = Locator.GetInstance(Of Session).User.Privileges.ProductGroupWrite
        BtnDelete.Visible = Locator.GetInstance(Of Session).User.Privileges.ProductGroupDelete
        BtnExport.Visible = Locator.GetInstance(Of Session).User.Privileges.SeveralExportGrid
    End Sub
    Private Sub Frm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        DgvProductGroupLayout.Load()
    End Sub
    Private Sub Form_Shown(sender As Object, e As EventArgs) Handles MyBase.Shown
        AddHandler Parent.FindForm.Resize, AddressOf FrmMain_ResizeEnd
    End Sub
    Private Sub BtnInclude_Click(sender As Object, e As EventArgs) Handles BtnInclude.Click
        Dim Form As New FrmProductGroup(New ProductGroup, Me)
        Form.ShowDialog()
    End Sub
    Private Sub BtnEdit_Click(sender As Object, e As EventArgs) Handles BtnEdit.Click
        Dim ProductsGroupForm As FrmProductGroup
        If DgvData.SelectedRows.Count = 1 Then
            Try
                Cursor = Cursors.WaitCursor
                _ProductsGroup = New ProductGroup().Load(DgvData.SelectedRows(0).Cells("id").Value, True)
                ProductsGroupForm = New FrmProductGroup(_ProductsGroup, Me)
                ProductsGroupForm.ShowDialog()
            Catch ex As Exception
                CMessageBox.Show("ERRO PG004", "Ocorreu um erro ao carregar o registro.", CMessageBoxType.Error, CMessageBoxButtons.OK, ex)
            Finally
    ...
    Private Sub BtnDelete_Click(sender As Object, e As EventArgs) Handles BtnDelete.Click
        If DgvData.SelectedRows.Count = 1 Then
            Try
                Cursor = Cursors.WaitCursor
                _ProductsGroup.Load(DgvData.SelectedRows(0).Cells("id").Value, False)
                If Not _ProductsGroup.LockInfo.IsLocked Then
                    If CMessageBox.Show("O registro selecionado será excluído. Deseja continuar?", CMessageBoxType.Question, CMessageBoxButtons.YesNo) = DialogResult.Yes Then
                        Try
                            _ProductsGroup.Delete()
                            _Filter.Filter()
                            DgvProductGroupLayout.Load()
                        ...
                        Catch ex As MySqlException
                            ...
                            Else
                                CMessageBox.Show("ERRO PG005", "Ocorreu um erro ao excluir o registro.", CMessageBoxType.Error, CMessageBoxButtons.OK, ex)
                            End If
                        End Try
                    End If
                Else
                    CMessageBox.Show(String.Format("Esse registro não pode ser excluído no momento pois está sendo utilizado por {0}.", GetTitleCase(_ProductsGroup.LockInfo.LockedBy.Value.Username)), CMessageBoxType.Information)
                End If
            Catch ex As Exception
                CMessageBox.Show("ERRO PG006", "Ocorreu um erro ao excluir o registro.", CMessageBoxType.Error, CMessageBoxButtons.OK, ex)
            Finally
                Cursor = Cursors.Default
            End Try
        End If
    ...
    Private Sub BtnExport_Click(sender As Object, e As EventArgs) Handles BtnExport.Click
        Dim Result As ReportResult = ExportGrid.Export({New ExportGrid.ExportGridInfo With {.Title = "Grupos de Produto", .Grid = DgvData}})
