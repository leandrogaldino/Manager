...
Public Class UcUserGrid
    ...
    Public Sub New()
        ...
        BtnInclude.Visible = Locator.GetInstance(Of Session).User.CanWrite(Routine.User)
        BtnEdit.Visible = Locator.GetInstance(Of Session).User.CanWrite(Routine.User)
        BtnDelete.Visible = Locator.GetInstance(Of Session).User.CanDelete(Routine.User)
    ...
    Private Sub BtnInclude_Click(sender As Object, e As EventArgs) Handles BtnInclude.Click
        Using Form As New FrmUser(New User, Me)
            Form.ShowDialog()
        End Using
    End Sub
    Private Sub BtnEdit_Click(sender As Object, e As EventArgs) Handles BtnEdit.Click
        If DgvData.SelectedRows.Count = 1 Then
            Try
                ...
                _User = New User().Load(DgvData.SelectedRows(0).Cells("id").Value, True)
                Using Form As New FrmUser(_User, Me)
                    Form.ShowDialog()
                End Using
            Catch ex As Exception
                CMessageBox.Show("ERRO US007", "Ocorreu um erro ao carregar o registro.", CMessageBoxType.Error, CMessageBoxButtons.OK, ex)
            Finally
    ...
    Private Sub BtnDelete_Click(sender As Object, e As EventArgs) Handles BtnDelete.Click
        If DgvData.SelectedRows.Count = 1 Then
            Try
                Cursor = Cursors.WaitCursor
                _User.Load(DgvData.SelectedRows(0).Cells("id").Value, False)
                If Not _User.LockInfo.IsLocked Then
                    If CMessageBox.Show("O registro selecionado será excluído. Deseja continuar?", CMessageBoxType.Question, CMessageBoxButtons.YesNo) = DialogResult.Yes Then
                        ...
                        Catch ex As MySqlException
                            ...
                            Else
                                CMessageBox.Show("ERRO US008", "Ocorreu um erro ao excluir o registro.", CMessageBoxType.Error, CMessageBoxButtons.OK, ex)
                            End If
                        End Try
                    End If
                Else
                    CMessageBox.Show(String.Format("Esse registro não pode ser excluído no momento pois está sendo utilizado por {0}.", _User.LockInfo.LockedBy.Value.Username.ToTitle()), CMessageBoxType.Information)
                End If
            Catch ex As Exception
                CMessageBox.Show("ERRO US009", "Ocorreu um erro ao excluir o registro.", CMessageBoxType.Error, CMessageBoxButtons.OK, ex)
            Finally
                Cursor = Cursors.Default
            End Try
        End If
    ...
    Private Sub BtnExport_Click(sender As Object, e As EventArgs) Handles BtnExport.Click
        Dim Result As ReportResult = ExportGrid.Export({New ExportGrid.ExportGridInfo With {.Title = "Usuários", .Grid = DgvData}})
