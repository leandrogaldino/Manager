...
Public Class FrmUserPrivilegePresets
    Private _UserPrivilegePreset As New UserPrivilegePreset
    ...
    Public Sub New()
        ...
        _Filter = New UserPrivilegePresetFilter(DgvData, PgFilter)
        _Filter.Filter()
        PgFilter.SelectedObject = _Filter
        BtnInclude.Visible = Locator.GetInstance(Of Session).User.Privilege.UserPrivilegePresetWrite
        BtnEdit.Visible = Locator.GetInstance(Of Session).User.Privilege.UserPrivilegePresetWrite
        BtnDelete.Visible = Locator.GetInstance(Of Session).User.Privilege.UserPrivilegePresetDelete
    End Sub
    Private Sub Frm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        DgvUserPrivilegePresetLayout.Load()
    End Sub
    Private Sub Form_Shown(sender As Object, e As EventArgs) Handles MyBase.Shown
        AddHandler Parent.FindForm.Resize, AddressOf FrmMain_ResizeEnd
    End Sub
    Private Sub BtnInclude_Click(sender As Object, e As EventArgs) Handles BtnInclude.Click
        Dim Form As New FrmUserPrivilegePreset(New UserPrivilegePreset, Me)
        Form.ShowDialog()
    End Sub
    Private Sub BtnEdit_Click(sender As Object, e As EventArgs) Handles BtnEdit.Click
        Dim UserPrivilegePresetForm As FrmUserPrivilegePreset
        If DgvData.SelectedRows.Count = 1 Then
            Try
                Cursor = Cursors.WaitCursor
                _UserPrivilegePreset = New UserPrivilegePreset().Load(DgvData.SelectedRows(0).Cells("id").Value, True)
                UserPrivilegePresetForm = New FrmUserPrivilegePreset(_UserPrivilegePreset, Me)
                UserPrivilegePresetForm.ShowDialog()
            Catch ex As Exception
                CMessageBox.Show("ERRO UP004", "Ocorreu um erro ao carregar o registro.", CMessageBoxType.Error, CMessageBoxButtons.OK, ex)
            Finally
    ...
    Private Sub BtnDelete_Click(sender As Object, e As EventArgs) Handles BtnDelete.Click
        If DgvData.SelectedRows.Count = 1 Then
            Try
                Cursor = Cursors.WaitCursor
                _UserPrivilegePreset.Load(DgvData.SelectedRows(0).Cells("id").Value, False)
                If Not _UserPrivilegePreset.LockInfo.IsLocked Then
                    If CMessageBox.Show("O registro selecionado será excluído. Deseja continuar?", CMessageBoxType.Question, CMessageBoxButtons.YesNo) = DialogResult.Yes Then
                        ...
                        Catch ex As MySqlException
                            CMessageBox.Show("ERRO UP005", "Ocorreu um erro ao excluir o registro.", CMessageBoxType.Error, CMessageBoxButtons.OK, ex)
                        End Try
                    End If
                Else
                    CMessageBox.Show(String.Format("Esse registro não pode ser excluído no momento pois está sendo utilizado por {0}.", GetTitleCase(_UserPrivilegePreset.LockInfo.LockedBy.Value.Username)), CMessageBoxType.Information)
                End If
            Catch ex As Exception
                CMessageBox.Show("ERRO UP006", "Ocorreu um erro ao excluir o registro.", CMessageBoxType.Error, CMessageBoxButtons.OK, ex)
            Finally
                Cursor = Cursors.Default
            End Try
        End If
    ...
    Private Sub BtnExport_Click(sender As Object, e As EventArgs) Handles BtnExport.Click
        Dim Result As ReportResult = ExportGrid.Export({New ExportGrid.ExportGridInfo With {.Title = "Predefiniçes Permissão de Usuário", .Grid = DgvData}})
