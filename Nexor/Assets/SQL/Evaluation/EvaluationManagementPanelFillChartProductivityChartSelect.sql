...
LEFT JOIN evaluation ON evaluation.id = evaluationtechnician.evaluationid AND evaluation.statusid = 1 AND evaluation.hasrepairid = @hasrepairid AND MONTHNAME(evaluation.evaluationdate) = @month AND YEAR(evaluation.evaluationdate) = @year
WHERE
